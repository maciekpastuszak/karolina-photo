...
router.route('/')
    ...
    .post(isLoggedIn, upload.array('images'), catchAsync(family.createFamilyPshoot));

router.get('/new', isLoggedIn, family.renderNewFamilyPshoot);

router.route('/:id')
    .get(catchAsync(family.showFamilyPshoot))
    .put(isLoggedIn, isOwnerFamily, upload.array('images'), catchAsync(family.editFamilyPshoot))
